// Page liste de produits (products.html) avec compteurs par article

document.addEventListener('DOMContentLoaded', () => {
    loadProductsWithCounters();
});

function loadProductsWithCounters() {
    fetch('../backend/products.php')
        ...
        .catch(error => {
            ...
            if (productsList) {
                productsList.innerHTML =
                    '<p class="text-center">Erreur lors du chargement des produits.</p>';
            }
        });
}
...
function attachAddToCartHandlers() {
    ...
    buttons.forEach(button => {
        button.addEventListener('click', async () => {
            ...
            try {
                // addToCart est async (vérifie aussi la connexion utilisateur)
